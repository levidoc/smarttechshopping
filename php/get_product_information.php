...
if (is_product_stock_available($product_index)){
    $replace = '                            
    <div>    
        <p class="single-product-pricing"><span></span> [PRODUCT_PRICE_]</p>
        <p>[PRODUCT_DESCRIPTION_]</p>
        <div class="single-product-form">
            
            <input id="stock_quantity" type="number" min="1" step="1" max="[STOCK_QUANTITY]" value="1">
            <br>
            <a onclick="process_cart()" class="cart-btn">
                <i class="fas fa-shopping-cart"></i> Add to Cart
            </a>
            <p><strong>Categories: </strong>[PRODUCT_CATEGORY_]</p>
        </div>
    </div>'; 
    ...
    <div>
        <div class="error">
            <div class="error__title"><i class="fas fa-exclamation-circle"></i>  Out Of Stock</div>
        </div>
...
if (file_exists($file_path)) {
    ...
    $subsearch_query = json_decode(file_get_contents($file_path), JSON_PRETTY_PRINT);
    ...
    foreach ($json_data as $row) {
        ...
        if ($product_index == $row['INDEX']) {
            ...
            if (retrieve_vendor_data($supplier_code) !== FALSE){
                ...
                $store_link = "store.php?reference=".encrypt_url($supplier_code);
    ...
    foreach ($subsearch_query as $product_search) {
        ...
        foreach ($x as $val){
            if (($relate_count < 3) && (strpos($product_category, $val) !== false) && (!empty($val)) && ($product_index !== $search_product_index)) {
    ...
    $data = str_ireplace(
        ...
        [$product_title, $price, $product_description, $product_image, $gallery_data, '', $product_category, $supplier_products,$related_product_data,$product_quantity],
        $html_structure,
    );
